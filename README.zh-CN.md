...
- 可在 GUI 中安全重启 TUN，或在不改变当前 TUN 会话的情况下重新执行连通检测。
- 可将 SwitchyOmega 条件写入 v2rayN 当前激活路由，自动备份原路由并重启 Bridge 加载；代理、直连规则分组排序。
- 路由管理器会列出 Bridge 已管理的 Proxy/Direct 内容，可复制、删改并与新条件合并去重后整体替换。
- 可开启和停止内存流量快照，列出诊断期间的全部目标域名、目标 IP、入口、出口、进程和连接数。
- 从纯 IP 的 TUN 连接中恢复 HTTP、TLS 与 QUIC 域名，使浏览器使用加密 DNS 时仍能应用域名路由。
- 对命中 Direct 的域名使用当前系统 DNS 重新解析，避免国内 CDN 沿用境外代理 DNS 的解析结果。
